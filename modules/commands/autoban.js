module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "NTKhang",
  description: "tự động cấm nhóm dùng bot nếu spam bot 3 lần/phút",
...
module.exports.handleEvent = async ({ Threads, api, event }) => {
  ...
  const thoi_gian_spam = 60; // 60000 millisecond (1 phút)
  ...
  if ((global.client.autobanthread[threadID].timeStart + thoi_gian_spam) <= Date.now()) {
    ...
    if (global.client.autobanthread[threadID].number >= so_lan_spam) {
      ...
      data.banned = true;
      data.reason = `spam bot ${so_lan_spam} lần/${thoi_gian_spam / 60}phút`;
      data.dateAdded = time;
