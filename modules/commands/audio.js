  ...
  let axios = require('axios');
    ...
    if (args.join(" ").indexOf("https://") == 0) {
        try {
            ...
            if (fs.statSync(path).size > 262144000000) return api.sendMessage('𝗞𝗵𝗼̂𝗻𝗴 𝘁𝗵𝗲̂̉ 𝗴𝘂̛̉𝗶 𝗳𝗶𝗹𝗲 𝗰𝗼́ 𝘁𝗵𝗼̛̀𝗶 𝗴𝗶𝗮𝗻 𝘁𝘂̛̀ 01:10:10 𝗩𝘂𝗶 𝗹𝗼̀𝗻𝗴 𝗰𝗵𝗼̣𝗻 𝗳𝗶𝗹𝗲 𝗸𝗵𝗼̂𝗻𝗴 𝗰𝗼́ 𝗮̂𝗺 𝘁𝗵𝗮𝗻𝗵.', event.threadID, () => fs.unlinkSync(path), event.messageID);
            return api.sendMessage({ 
                body: `🎶== [ 𝗧𝗛𝗘̂́ 𝗚𝗜𝗢̛́𝗜 𝗔̂𝗠 𝗡𝗛𝗔̣𝗖 ] ==🎶\n━━━━━━━━━━━━━━━━\n→ [🎵] 𝗕𝗮̀𝗶 𝗵𝗮́𝘁: ${data.title} ( ${this.convertHMS(data.dur)} )\n→ [📎] 𝗡𝗴𝗮̀𝘆 𝘁𝗮̉𝗶 𝗹𝗲̂𝗻: ${data.uploadDate}\n→ [🔎] 𝗧𝗲̂𝗻 𝗸𝗲̂𝗻𝗵: ${data.author} ( ${data.sub} )\n→ [🌐] 𝗟𝘂̛𝗼̛̣𝘁 𝘅𝗲𝗺: ${data.viewCount}\n→ [👍] 𝗟𝘂̛𝗼̛̣𝘁 𝘁𝗵𝗶́𝗰𝗵: ${data.likes}\n→ [⏰] 𝗧𝗵𝗼̛̀𝗶 𝗴𝗶𝗮𝗻 𝘅𝘂̛̉ 𝗹𝗶́: ${Math.floor((Date.now()- data.timestart)/1000)} giây\n━━━━━━━━━━━━━━━━\n=== [ 𝗠𝗜𝗥𝗔𝗜 𝗕𝗼𝘁 𝗩𝟯 ] ===`,
                attachment: fs.createReadStream(path)}, event.threadID, ()=> fs.unlinkSync(path), 
            event.messageID)
