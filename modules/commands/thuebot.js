...
exports.run = function(o) {
  ...
  switch (o.args[0]) {
    case 'add': {
      if (!o.args[1])return send(`➜ Sử dụng {prefix}{command name} add {ID thread} {ID user} {time start} => {time end}`);
    ...
    default: send(`=== 『 SUPPORT 』 ===\n━━━━━━━━━━━━━━━━\n➜ Dùng: ${global.config.PREFIX}${this.config.name} add → Để thêm nhóm vào danh sách thuê bot\n➜ Dùng: ${global.config.PREFIX}${this.config.name} list → Để xem danh sách thuê bot\n➜ 𝗛𝗗𝗦𝗗 → ${global.config.PREFIX}${this.config.name} lệnh cần dùng.`)
      break;
  }
  save();
};
exports.handleReply = async function(o) {
  ...
  if (isFinite(o.event.args[0])) {
    ...
    if (!info)return send(`➜ Số thứ tự không tồn tại!`);
