...
attachment: imag 
    ...
    if (z == args[0]) {
      ...
      return api.sendMessage(`Bạn đã thắng với con ngựa ${args[0]} và nhận được đúng cái nịt.`, threadID, messageID)
    } else {
      await Currencies.decreaseMoney(senderID, parseInt(sotien));
      return api.sendMessage(`Bạn đã thua và con ngựa ${z} về trước và mất số tiền 1000$.`, threadID, messageID)
    }
  })
}
}
