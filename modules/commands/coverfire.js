...
color = args.join(" ").trim().replace(/\s+/g, " ").replace(/(\s+\|)/g, "|").replace(/\|\s+/g, "|").split("|")[3] || "red";
  ...
  if (!fs.existsSync(__dirname +
    ...
    fs.writeFileSync(__dirname + `/tad/GCatamaran-Regular.ttf`, Buffer.from(getfont, "utf-8"));
  };
  if (!fs.existsSync(__dirname +
...
ctx.drawImage(canvas2 , 0, 0, canvas.width, canvas.height)
   Canvas.registerFont(__dirname + `/tad/GCatamaran-Regular.ttf`, {
