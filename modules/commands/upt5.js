const fbname = "kzkhanh547";
const githubname = "kzkhanh";
...
const time = process.uptime(),
  ...
  var id = Math.floor(Math.random() * 848) +1
    ...
    const lengthchar = (await axios.get('http://docs-api.vorhaidang0123.repl.co/taoanhdep/data')).data
...
registerFont(__dirname + `/tad/CaviarDreams.ttf`, {
    ...
    ctx.textAlign = "start";
    ctx.font = "35px time";
    ctx.fillText("@" + "", 215, 220)
    ctx.fillText("@" + "", 215, 275)
    ctx.fillText("@" + "", 215, 335)
    ctx.restore();
    ctx.save();
    ctx.beginPath();
