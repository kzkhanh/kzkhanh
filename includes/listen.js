module.exports = function({ api, models }) {
 ...
 return async (event) => {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`=====『 𝗞𝗜𝗘̂̉𝗠 𝗗𝗨𝗬𝗘̣̂𝗧 』=====\nNhóm bạn chưa được admin duyệt. Để gửi yêu cầu duyệt, hãy chat: duyetbox`, event.threadID);
    }
    };
    switch (event.type) {
      //<--Nhận, xử lí dữ liệu-->//
